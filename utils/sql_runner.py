...
def _split_sqlplus(script: str) -> list[str]:
    ...
    Понимает три вида разделителей:
    1. '/ --HARDCODE FOR DEPLOYMENT' — в конце строки или на отдельной строке
    2. '/' на отдельной строке — стандартный PL/SQL разделитель
    Внутри PL/SQL-блоков (PACKAGE, PACKAGE BODY, PROCEDURE, FUNCTION, TRIGGER,
    BEGIN...END) точки с запятой не режут — блок идёт целиком до '/'.
    """
    import re
    ...
    # Хвост: может содержать обычные DDL через ";"
    # Но если это PL/SQL-блок (PACKAGE/PROCEDURE/FUNCTION/TRIGGER/BEGIN),
    # передаём целиком без разбивки по ";"
    tail = "\n".join(buf).strip()
    if tail:
...
def _split_go(script: str) -> list[str]:
    """MSSQL: 'GO' на отдельной строке (регистронезависимо)."""
    statements: list[str] = []
    buf: list[str] = []
    for line in script.splitlines():
...
def _split_semicolon(script: str) -> list[str]:
    # Простое разбиение, игнорируя ";" внутри кавычек и комментариев -- (строчных).
    statements: list[str] = []
    buf: list[str] = []
...
class SQLRunner:
    ...
    def run_dir(self, dir_path: Path, pattern: str = "*.sql") -> int:
        """Выполняет все .sql-файлы в каталоге по алфавиту (удобно для 01_..., 02_...)."""
